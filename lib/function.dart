...
import 'package:image/image.dart' as iml; 

const String keyPalette = 'palette';
const String keyNoOfItems = 'noIfItems';

int numberOfPixelsPerAxis = 12;
...
List<Color> extractPixelsColors(Uint8List? bytes) {
  ...
  iml.Image? image = iml.decodeImage(values);
  ...
  int xChunk = width! ~/ (numberOfPixelsPerAxis + 1);
  int yChunk = height! ~/ (numberOfPixelsPerAxis + 1);

  for (int j = 1; j < numberOfPixelsPerAxis + 1; j++) {
    for (int i = 1; i < numberOfPixelsPerAxis + 1; i++) {
